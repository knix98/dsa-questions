...
void pushdown(int pos, int beg, int end) {
	if(lazy[pos] != 0) {  // because -ve update values can also be there
		st[pos] += lazy[pos];
		if(beg != end) {
...
signed main() {
	...
	while(q--) {
		...
		char ch;
		cin>>l>>r;
		ch = cin.get();  // question states: if 3 values given in 1 line, that means update range query, otherwise min range query
		if(ch == ' ') {  // means 3 values are there, so update query
			cin>>v;
			if(l <= r) {
				update(0, n-1, l, r, v, 0);
			} else {  // means a circular range, so break it down into 2 simpler ranges : [l,n-1] and [0,r]
				update(0, n-1, l, n-1, v, 0);
				update(0, n-1, 0, r, v, 0);
			}
		} else {  // means 2 values are there, so min range query
			if(l <= r) {
